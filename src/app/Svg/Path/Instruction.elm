module Svg.Path.Instruction exposing (ArcFlag(..), CurveContinuation(..), Direction(..), DrawInstruction(..), Instruction(..), ManyInstruction(..), fixed, formatPointWithPrecision, instructionToString, mapAbsolute, mapAbsoluteCurveCont, propagate, roundToAtMost)
...
fixed i n =
    let
        ...
        nInt =
            round (n * pow)
    in
    Debug.toString (toFloat nInt / pow)
...
propagate instruction ({ start, current } as st) =
    let
        ( cx, cy ) =
            current
    ...
    case instruction of
        Absolute (Move p) ->
            { st | current = p, direction = Point.direction p st.current }

        Relative (Move p) ->
            { st | current = Point.add p current }

        Absolute (Line p) ->
            { st | current = p, direction = Point.direction current p }

        Relative (Line p) ->
            { st | current = Point.add current p }

        Absolute (Vertical y) ->
            { st | current = Point.point cx y, direction = Point.direction current (Point.point cx y) }

        Relative (Vertical y) ->
            { st | current = Point.point cx (cy + y) }

        Absolute (Horizontal x) ->
            { st | current = Point.point x cy, direction = Point.direction current (Point.point x cy) }

        Relative (Horizontal x) ->
            { st | current = Point.point (cx + x) cy }

        Absolute (Many (LineMany ps)) ->
            case List.drop (List.length ps - 2) ps of
                [ q, p ] ->
                    { st | current = p, direction = Point.direction q p }

                [ p ] ->
                    { st | current = p, direction = Point.direction st.current p }
        ...
        Relative (Many (LineMany ps)) ->
            let
                newState =
                    List.foldl (\rel ns -> { ns | current = Point.add rel ns.current }) st ps
            in
            case List.drop (List.length ps - 2) ps of
                [ q, p ] ->
        ...
        Relative (Arc radii xAngleRotation flags p) ->
            let
                new =
                    Point.add current p
            in
            { st | current = new, direction = Point.direction current new }
        ...
        Absolute (Many (QuadraticMany control goal continuations)) ->
            let
                newState =
                    List.foldl resultContinuation { st | current = goal } continuations
            in
        ...
        Relative (Many (QuadraticMany control goal continuations)) ->
            let
                newState =
                    List.foldl resultContinuation { st | current = Point.add current goal } continuations
            in
            { newState | direction = Point.direction current newState.current }
        ...
        ClosePath ->
            { st | current = start, direction = Point.direction current start }
