module Svg.Geometry exposing (Diagram(..), circle, compose, ellipse, line, ngon, polygon, polyline, rect, runDiagram, sameOrigin, square, triangle)
...
ngon sides radius rotation ( x, y ) =
    let
        ...
        points =
            List.map (makeTri << toFloat) (List.range 0 (sides - 1))
    in
    polygon points
